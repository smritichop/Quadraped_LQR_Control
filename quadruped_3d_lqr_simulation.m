...
%% ============================================================
% Augment with integral states for zero steady-state error
...
% Integral: [int_φ, int_θ, int_ψ, int_px, int_py, int_pz]

% Q matrix for states - penalize deviations
Q_state = diag([2000,   ... % roll - HIGH (stability critical)
...
% Q matrix for integral states
Q_int = diag([300,     ... % integral roll
              300,     ... % integral pitch
              150,     ... % integral yaw
              100,     ... % integral px
              100,     ... % integral py
              400]);   ... % integral pz (height)
...
% Extract gains
K_x = K_lqr_aug(:, 1:nx);       % 12 x 12
K_i = K_lqr_aug(:, nx+1:end);   % 12 x 6
...
for k = 1:(N-1)
    ...
    % Tracking error for integral (position/orientation only)
    e_track = C_y * e_state;
    
    % Update integral with CORRECT SIGN (must match augmented system formulation)
    % The augmented system has ż_int = -C_y*x, so we integrate NEGATIVE error
    z_int = z_int - e_track * dt;  % MINUS sign to match A_aug = [...; -C_y, ...]
    z_int = max(min(z_int, 1.0), -1.0);  % Anti-windup clamp
...
for k = 1:(N-1)
    ...
    z_int_dist = z_int_dist - e_track * dt;  % MINUS sign to match A_aug formulation
    z_int_dist = max(min(z_int_dist, 1.0), -1.0);  % Anti-windup clamp
